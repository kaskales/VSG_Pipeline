...
for file in arguments.s: # could be putting in multiple sequencing data files
	...
	subprocess.call(['Trinity --seqType fq --max_memory 10G --full_cleanup --single ' + timeRan + "/"+ filenameS+"/"+str(filenameS) +'_trimmed2.fq ' +'--min_contig_length ' + str(min_pro_len*3) + ' --no_normalize_reads --output ' + timeRan+"/"+filenameS+"_Trinity/"], shell=True)
...
# make bed
record_dict = SeqIO.index(currDir +'/' + timeRan + "/"+timeRan+"_orf_VSGs.fa","fasta")
