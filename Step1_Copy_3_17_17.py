			...
			for frame in range(3): #loops 3 times, frame will start at 0, 1, and 2
				...
				while aa_start < max_aa_start: # while the starting amino acid is less than the peptide sequence length
					...
					if trans_start == -1: # if no start is found
						trans_end = trans.find("*", aa_start) # research for the end starting at aa_start 
						if trans_end == -1: # if no end is found
							aa_start = trans_len # sets the start to the last index to end loop
							...
							if trans_end - aa_start > min_pro_len: #if length from aa_start to the stop codon found is > min length
								...
								if strand==1: # adds to dictionary for ORFs of 1 strands
									...
									sequence = record_dict[record].seq[aa_start:end]
									orf_dict[record_dict[record].id].append((sequence,aa_start,end,True)) # check datastructure append
									ORF_seq = SeqRecord(sequence, id=record_dict[record].id+'_'+str(aa_start)+'_'+str(end))
								if strand==-1: # adds to dictionary for ORFs of -1 strands
